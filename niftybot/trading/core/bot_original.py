...
Logic aligned closer to original file-based version
Updated: realistic entry window, full exit conditions, defensive rolling, final_margin usage
+ Stop check from website + wider test window + relaxed VIX + improved order logging + API throttling
+ Improved short strike selection (nearest valid ±150 with live premium)
+ FIXED: Save daily_profit_target & daily_stop_loss to BotStatus model for UI display
+ IMPROVED: MAX_LOTS_HARD_CAP now dynamic — read from per-user BotStatus.max_lots_hard_cap
"""
import time
import sys
import traceback
...
class Config:
    ...
    # TEMPORARY WIDENED WINDOW FOR TESTING TODAY (Monday)
    # Change back to 9:21:30–9:22:00 after successful test
    ENTRY_START = dtime(9, 21, 0) # widened for testing
    ENTRY_END = dtime(9, 22, 0) # widened for testing
    ...
    MAX_LOTS = 50
    # MAX_LOTS_HARD_CAP removed — now dynamic from BotStatus per user
    VIX_EXIT_ABS = 18.0
    VIX_SPIKE_MULTIPLIER = 1.20
  
    # Relaxed VIX range (matches working version)
    VIX_MIN = 7.0
    VIX_MAX = 30.0
...
class Engine:
    ...
    def get_available_capital_debug(self):
        """Fetch and log full capital details for preview/debug"""
        try:
            ...
            self.logger.info("CURRENT AVAILABLE CAPITAL (ZERODHA API)", {
            ...
            self.logger.warning("Failed to fetch current capital", {"error": str(e)})
            return 0.0

    def capital_available(self) -> float:
        """Returns TOTAL usable margin = live_balance + collateral"""
        try:
            margins = self.kite.margins()["equity"]
            live_balance = margins["available"].get("live_balance", 0)
            cash = margins["available"].get("cash", 0)
    ...
    def spot(self) -> Optional[float]:
        time.sleep(0.5) # Throttle to avoid "Too many requests"
        try:
    ...
    def ltp(self, symbol: str) -> float:
        time.sleep(0.4) # Throttle every option quote
        try:
    ...
    def find_short_strike(self, atm_strike: int, cp: str) -> int:
        """
        Find nearest valid short strike around ATM ±150 with live premium
        """
        ...
        for offset in range(50, 501, 50):
            for sign in [1, -1]:
                test_strike = target + sign * offset
                # Prevent crossing ATM
    ...
    def find_hedge_strike(self, short_strike: int, cp: str, common_target_prem: float, simulate: bool = False) -> int:
        ...
        for _, row in df.iterrows():
            ...
            if prem <= 3.0: # require hedges to have at least ₹3
                continue
    ...
    def lock_target(self, target_rupee: float):
        ...
        # FIXED: Save to BotStatus model so UI can display updated target & stop loss
        self.bot_status.daily_profit_target = rounded_target
        self.bot_status.daily_stop_loss = -rounded_target
        self.bot_status.save(update_fields=['daily_profit_target', 'daily_stop_loss'])
    ...
    def enter(self) -> bool:
        ...
        if self.state.data.get("trade_active"):
            self.logger.info("Trade already active - skipping new entry")
            return False
        if self.state.data["trade_taken_today"]:
            return False
        ...
        if expiry == today:
            ...
            return False
        # Tuesday skip commented out for testing
        # today_weekday = datetime.now(Config.TIMEZONE).weekday()
        # if today_weekday == 1:
        #     self.logger.info("ENTRY SKIPPED", {"reason": "User-defined rule: Skip all trades on Tuesdays"})
        #     return False
        vix_val = self.vix()
    ...
    def check_and_adjust_defensive(self) -> bool:
        ...
        if (spot >= ce_short + Config.ADJUSTMENT_TRIGGER_POINTS and
            ...
            if not new_sym or self.ltp(new_sym) <= 0:
                self.order(old_sym, "SELL", qty)
                self.state.data["realized_pnl"] -= realized
                for leg in algo_legs.values():
                    if leg["symbol"] == old_sym and leg["status"] == "CLOSED":
